...
HandleWebpage::HandleWebpage() { _webServer = new ESP8266WebServer(80); }
...
void HandleWebpage::handleSyncTime() {
  ...
  if (_callBackAdjustTime != nullptr) {
    ...
    Serial.printf("set time to: %02d.%02d.%d %02d:%02d:%02d\n", recivedTime.day(), recivedTime.month(),
                  recivedTime.year(), recivedTime.hour(), recivedTime.minute(), recivedTime.second());
    _webServer->send(200, "text/plane", "{\"success\": true}");
  } else {
