...
void MainFunc::setInConfigMode(bool value) { _inConfigMode = value; }
...
void MainFunc::setWlanStatusOnDisplay(bool wlan) { _handleDisplay->setWlanStatus(wlan); }
...
void MainFunc::storeValuesInEeprom() { _handleEeprom->commitValues(); }

void MainFunc::webpageCheckClient() { _handleWebpage->handleClient(); }

bool MainFunc::isSoundRunning() { return _handleAudio->isSoundPlaying(); }
...
void MainFunc::setupEeprom() {
  ...
  if ((displayMode != DM_ON) && (displayMode != DM_OFF) && (displayMode != DM_ON_AT_BEDTIME) &&
      (displayMode != DM_ON_AT_DAYTIME)) {
...
void MainFunc::setDisplayMode(uint8_t displayMode) { _handleEeprom->setDisplayMode(displayMode); }

void MainFunc::setSoundReplay(uint8_t soundReplay) { _handleEeprom->setSoundReplay(soundReplay); }
...
uint8_t MainFunc::getSunBrightness() { return MainFunc::_sunBrightness; }

uint8_t MainFunc::getMoonBrightness() { return MainFunc::_moonBrightness; }

uint8_t MainFunc::getGainSound() { return _handleAudio->getGain(); }

uint8_t MainFunc::getDisplayMode() { return _handleEeprom->getDisplayMode(); }

uint8_t MainFunc::getSoundReplayes() { return _handleEeprom->getSoundReplay(); }

int MainFunc::procent2brightness(int prozent) { return static_cast<int>(prozent * 10.23); }
...
uint16_t MainFunc::getTimeInMintuesPerDay(int hour, int minutes) { return (hour * 60 + minutes); }
...
uint16_t MainFunc::getTimeInMinutesPerDay(uint16_t eeWakeUpTimeFormat) { return eeWakeUpTimeFormat >> 4; }

TimeTypes MainFunc::getWakeUpType(uint16_t eeWakeUpTimeFormat) { return (TimeTypes)(eeWakeUpTimeFormat & 0x000F); }
...
void MainFunc::updateDisplay() { updateDisplay(false); }
...
void MainFunc::updateDisplay(bool force) {
  ...
  if (((_milliTime - _lastMicroTime) > 2000) || force) {
    ...
    if ((_lastHour != hour) || (_lastMinute != minute) || force) {
      ...
      char timeStr[MAX_CHARACTOR_TIME_STR];
      snprintf(timeStr, MAX_CHARACTOR_TIME_STR, "%02d:%02d", hour, minute);
