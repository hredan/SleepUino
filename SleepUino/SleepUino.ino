...
void loop() {
  static states_t STATE = OPS;  // current state machine state, with static this
                                // variable will be initialzied only one time
  ...
  switch (STATE) {
    ...
    // this is a transition state where we start the fast blink as feedback to
    // the user, but we also need to wait for the user to release the button,
    // i.e. end the long press, before moving to the BLINK state.
    case TO_OPS:
      if (myBtn.wasReleased()) {
    ...
    // turn the LED off (as feedback to the user) and move to the TO_ONOFF
    // state.
    case WIFI:
    ...
    // this is a transition state where we just wait for the user to release the
    // button before moving back to the ONOFF state.
    case TO_WIFI:
      if (myBtn.wasReleased()) {
        STATE = WIFI;
