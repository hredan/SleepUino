...
HandleEeprom::HandleEeprom()
{
  EEPROM.begin(512);
};

uint8_t HandleEeprom::getSunBrightnessValue()
{
  return EEPROM.read(EEADR_BRI_SUN);
}

uint8_t HandleEeprom::getMoonBrightnessValue()
{
  return EEPROM.read(EEADR_BRI_MOON);
}

uint8_t HandleEeprom::getGainValue()
{
  return EEPROM.read(EEADR_GAIN_SOUND);
}

uint8_t HandleEeprom::getDisplayMode()
{
  return EEPROM.read(EEADR_DISPLAY_MODE);
}

uint8_t HandleEeprom::getSoundReplay()
{
  return EEPROM.read(EEADR_SOUND_REPLAY);
}
...
bool HandleEeprom::getUpdateEeprom()
{
  return _updateEeprom;
}
...
uint8_t HandleEeprom::getNumberOfWakeTimes()
{
  return EEPROM.read(EEADR_COUNT);
}